...
// Actualizamos la validación para exigir espesor y sistema si es cancel corredizo
...
const enlaceWhatsApp = computed(() => {
  ...
  return `https://wa.me/522299208830?text=${encodeURIComponent(mensaje)}`
})
</script>
        ...
        <div class="max-w-md mx-auto space-y-6">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <div class="relative"><input type="number" min="0" @keydown="bloquearCaracteres" v-model.number="store.proyecto.medidas.ancho" class="block w-full rounded-lg border-slate-300 border px-4 py-3 focus:border-blue-600 focus:ring-blue-600 text-lg transition-colors outline-none" placeholder="Ej. 120"></div>
            </div>
            <div>
              <label class="block text-sm font-bold text-slate-700 mb-2">Alto (cm)</label>
              <div class="relative"><input type="number" min="0" @keydown="bloquearCaracteres" v-model.number="store.proyecto.medidas.alto" class="block w-full rounded-lg border-slate-300 border px-4 py-3 focus:border-blue-600 focus:ring-blue-600 text-lg transition-colors outline-none" placeholder="Ej. 210"></div>
            </div>
          </div>
          ...
          <div v-if="!errorMedidas" class="mt-6 border-t border-slate-100 pt-6 animate-fade-in">
            ...
            <div class="space-y-3">
              <label class="flex items-start cursor-pointer group p-3 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors" :class="{'bg-blue-50 border-blue-200': tipoMedida === 'fabricacion'}">
                ...
                <span class="ml-3"><span class="block text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors">Medidas de fabricación</span><span class="block text-xs text-slate-500 mt-1">Son las medidas finales exactas. No se hará ningún tipo de descuento.</span></span>
              </label>
              <label class="flex items-start cursor-pointer group p-3 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors" :class="{'bg-blue-50 border-blue-200': tipoMedida === 'vano'}">
                ...
                <span class="ml-3"><span class="block text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors">Medidas de vano</span><span class="block text-xs text-slate-500 mt-1">Son las medidas del hueco. Se harán los descuentos correspondientes de holguras y herrajes.</span></span>
              </label>
            </div>
          </div>
          <div v-if="tipoMedida && !errorMedidas" class="mt-4 p-4 bg-blue-50/50 border border-blue-100 rounded-lg animate-fade-in">
            <label class="flex items-center cursor-pointer">
              ...
              <span class="ml-3 text-sm font-bold text-blue-900 leading-tight">Acepto que las medidas proporcionadas son de {{ tipoMedida === 'fabricacion' ? 'fabricación' : 'vano' }}.</span>
            </label>
          </div>
        </div>
      ...
      <div v-else-if="store.pasoActual === 3" class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 animate-fade-in max-w-2xl mx-auto">
        ...
        <div class="space-y-8">
          <div v-if="store.proyecto.tipoInstalacion === 'cancel'" class="space-y-8 animate-fade-in">
            <div>
              <label class="block text-base font-bold text-slate-700 mb-4">¿Qué tipo de apertura requiere tu cancel?</label>
            ...
            <div v-if="store.proyecto.detalles.tipoCancel === 'corredizo'" class="space-y-6 animate-fade-in border-t border-slate-100 pt-6">
              <div>
                ...
                <div class="flex gap-4">
                  <label class="flex-1 cursor-pointer group p-3 rounded-xl border-2 text-center transition-all" :class="store.proyecto.detalles.espesor === '6mm' ? 'border-blue-600 bg-blue-50' : 'border-slate-200 hover:bg-slate-50'">
                    <input type="radio" v-model="store.proyecto.detalles.espesor" value="6mm" @change="store.proyecto.detalles.sistemaCorredizo = ''" class="hidden">
                    <span class="font-bold text-slate-800">6 mm</span>
                  </label>
                  <label class="flex-1 cursor-pointer group p-3 rounded-xl border-2 text-center transition-all" :class="store.proyecto.detalles.espesor === '10mm' ? 'border-blue-600 bg-blue-50' : 'border-slate-200 hover:bg-slate-50'">
                    <input type="radio" v-model="store.proyecto.detalles.espesor" value="10mm" @change="store.proyecto.detalles.sistemaCorredizo = ''" class="hidden">
                    <span class="font-bold text-slate-800">10 mm</span>
                  </label>
                </div>
              </div>
              ...
              <div v-if="store.proyecto.detalles.espesor" class="animate-fade-in">
                ...
                <div class="relative">
                  <select v-model="store.proyecto.detalles.sistemaCorredizo" class="block w-full rounded-lg border-slate-300 border px-4 py-3 focus:border-blue-600 focus:ring-blue-600 transition-colors outline-none bg-white font-medium text-slate-700 appearance-none cursor-pointer">
          ...
          <div v-if="['barandal', 'formas'].includes(store.proyecto.tipoInstalacion)" class="space-y-6 animate-fade-in">
            <div>
              <label class="block text-base font-bold text-slate-700 mb-4">Para fabricar esta pieza con precisión, ¿enviarás alguna plantilla física o digital?</label>
              <div class="flex gap-4">
                <label class="flex-1 cursor-pointer group p-4 rounded-xl border-2 text-center transition-all" :class="store.proyecto.detalles.plantilla === 'si' ? 'border-blue-600 bg-blue-50' : 'border-slate-200 hover:bg-slate-50'">
                  ...
                  <span class="block text-2xl mb-2">✅</span>
                  <span class="font-bold text-slate-800">Sí, enviaré plantilla</span>
                </label>
                <label class="flex-1 cursor-pointer group p-4 rounded-xl border-2 text-center transition-all" :class="store.proyecto.detalles.plantilla === 'no' ? 'border-blue-600 bg-blue-50' : 'border-slate-200 hover:bg-slate-50'">
                  ...
                  <span class="block text-2xl mb-2">❌</span>
                  <span class="font-bold text-slate-800">No tengo plantilla</span>
                </label>
              </div>
            </div>
          </div>
      ...
      <div v-else-if="store.pasoActual === 4" class="animate-fade-in">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
          <button v-for="vidrio in vidrios" :key="vidrio.id" @click="seleccionarVidrio(vidrio.nombre)" :class="['bg-white border-2 rounded-xl overflow-hidden shadow-sm transition-all duration-300 flex flex-col text-left group', store.proyecto.tipoVidrio === vidrio.nombre ? 'border-blue-600 ring-4 ring-blue-600/20 transform scale-[1.02]' : 'border-slate-200 hover:border-blue-400 hover:shadow-lg']">
            <div class="h-24 w-full flex items-center justify-center border-b border-slate-100 bg-slate-50"><span :class="vidrio.logoStyle">{{ vidrio.logoTxt }}</span></div>
            <div class="p-5"><h3 class="text-lg font-bold text-slate-900 mb-1">{{ vidrio.nombre }}</h3><p class="text-sm text-slate-500 leading-relaxed line-clamp-3">{{ vidrio.desc }}</p></div>
          </button>
        </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-6">
          ...
          <a v-if="store.proyecto.tipoVidrio" :href="enlaceWhatsApp" target="_blank" class="px-8 py-4 rounded-lg font-extrabold text-white bg-[#25D366] hover:bg-[#1EBE57] shadow-lg shadow-[#25D366]/30 transform hover:-translate-y-1 transition-all duration-300 flex items-center gap-3 w-full sm:w-auto justify-center animate-fade-in">
